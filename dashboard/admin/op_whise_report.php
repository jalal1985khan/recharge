...
<head>
    <title>Operator Whise Report</title>
                                                        ...
                                                        <tbody>
                                                           <?php
                                                            if(isset($_POST['filter_date'])){
                                                                $date1 = $_POST['date1'];
                                                      $query =  $con->query("SELECT DISTINCT OP, SUM(AMOUNT) FROM recharge_history WHERE DATE LIKE '$date1' GROUP BY OP ORDER BY OP DESC;");
                                                                    while($OP = $query->fetch_assoc()){
                                                                        $total_sum += $OP['SUM(AMOUNT)'];
                                                                        ?>
                                                                        <tr>
                                                                          <td><?php echo $OP['OP'] ?></td>
                                                                          <td><?php echo $OP['SUM(AMOUNT)'] ?></td>
                                                                        </tr>
                                                                   <?php }
                                                                   
                                                            }else{ 
                                                            $date = date("Y-m-d");
                                                      $query =  $con->query("SELECT DISTINCT OP, SUM(AMOUNT) FROM recharge_history WHERE DATE LIKE '$date' GROUP BY OP ORDER BY OP DESC;");
                                                                    while($OP = $query->fetch_assoc()){
                                                                        $total_sum += $OP['SUM(AMOUNT)'];
                                                                        ?>
                                                                        <tr>
