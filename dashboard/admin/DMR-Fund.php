...
if(isset($_POST['masterdistributer'])){
    ...
    if($type == "add"){
        ...
        if($con->query("UPDATE masterdistributer SET DMRBAL='$total_amount' WHERE ID='$ms_id'")){
            $admin = $con->query("select * from admin where ID='$admin_id'")->fetch_assoc();
            $admin_rc = $admin['DMRBAL'];
            $ad_am = $admin_rc - $amount;
            if($con->query("UPDATE admin SET DMRBAL='$ad_am' WHERE ID='$admin_id'") && $ad_am > 0){
            $con->query("INSERT INTO `fund_transfer`(`USER`, `TYPE`, `AMOUNT`) VALUES('MASTERDISTRIBUTER' , 'ADD' , '$amount')");
     ...
     if($type == "deduct"){
        ...
        $user_rc = $user['DMRBAL'];
        $total_amount = $user_rc - $amount;
        if($con->query("UPDATE masterdistributer SET DMRBAL='$total_amount' WHERE ID='$ms_id'") && $total_amount > 0){
            $admin = $con->query("select * from admin where ID='$admin_id'")->fetch_assoc();
            $admin_rc = $admin['DMRBAL'];
            $ad_am = $admin_rc + $amount;
            if($con->query("UPDATE admin SET DMRBAL='$ad_am' WHERE ID='$admin_id'")){
            $con->query("INSERT INTO `fund_transfer`(`USER`, `TYPE`, `AMOUNT`) VALUES('MASTERDISTRIBUTER' , 'DEDUCT' , '$amount')");
                echo "<script> alert('Amount Has Been Added.') </script>";
            }
        }else{
...
if(isset($_POST['distributer'])){
    ...
    if($type == "add"){
        ...
        $user_rc = $user['DMRBAL'];
        $total_amount = $user_rc + $amount;
        if($con->query("UPDATE distributer SET DMRBAL='$total_amount' WHERE ID='$ms_id'")){
            $admin = $con->query("select * from admin where ID='$admin_id'")->fetch_assoc();
            $admin_rc = $admin['DMRBAL'];
            $ad_am = $admin_rc - $amount;
            if($con->query("UPDATE admin SET DMRBAL='$ad_am' WHERE ID='$admin_id'") && $ad_am > 0){
            $con->query("INSERT INTO `fund_transfer`(`USER`, `TYPE`, `AMOUNT`) VALUES('DISTRIBUTER' , 'ADD' , '$amount')");
                echo "<script> alert('Amount Has Been Added.') </script>";
            }else{
                echo "<script> alert('You have Not Sufficient Amount.') </script>";
            }
     ...
     if($type == "deduct"){
        ...
        $user_rc = $user['DMRBAL'];
        $total_amount = $user_rc - $amount;
        if($con->query("UPDATE distributer SET DMRBAL='$total_amount' WHERE ID='$ms_id'") && $total_amount > 0){
            $admin = $con->query("select * from admin where ID='$admin_id'")->fetch_assoc();
            $admin_rc = $admin['DMRBAL'];
            $ad_am = $admin_rc + $amount;
            if($con->query("UPDATE admin SET DMRBAL='$ad_am' WHERE ID='$admin_id'")){
            $con->query("INSERT INTO `fund_transfer`(`USER`, `TYPE`, `AMOUNT`) VALUES('DISTRIBUTER' , 'DEDUCT' , '$amount')");
                echo "<script> alert('Amount Has Been Added.') </script>";
            }
        }else{
                echo "<script> alert('User Has Not Sufficient Amount.') </script>";
...
if(isset($_POST['retailer'])){
    ...
    if($type == "add"){
        ...
        $user_rc = $user['DMRBAL'];
        $total_amount = $user_rc + $amount;
        if($con->query("UPDATE retailer SET DMRBAL='$total_amount' WHERE ID='$ms_id'")){
            $admin = $con->query("select * from admin where ID='$admin_id'")->fetch_assoc();
            $admin_rc = $admin['DMRBAL'];
            $ad_am = $admin_rc - $amount;
            if($con->query("UPDATE admin SET DMRBAL='$ad_am' WHERE ID='$admin_id'") && $ad_am > 0){
            $con->query("INSERT INTO `fund_transfer`(`USER`, `TYPE`, `AMOUNT`) VALUES('RETAILER' , 'ADD' , '$amount')");
                echo "<script> alert('Amount Has Been Added.') </script>";
            }else{
                echo "<script> alert('You have Not Sufficient Amount.') </script>";
            }
     ...
     if($type == "deduct"){
        ...
        $user_rc = $user['DMRBAL'];
        $total_amount = $user_rc - $amount;
        if($con->query("UPDATE retailer SET DMRBAL='$total_amount' WHERE ID='$ms_id'") && $total_amount > 0){
            $admin = $con->query("select * from admin where ID='$admin_id'")->fetch_assoc();
            $admin_rc = $admin['DMRBAL'];
            $ad_am = $admin_rc + $amount;
            if($con->query("UPDATE admin SET DMRBAL='$ad_am' WHERE ID='$admin_id'")){
                $con->query("INSERT INTO `fund_transfer`(`USER`, `TYPE`, `AMOUNT`) VALUES('RETAILER' , 'DEDUCT' , '$amount')");
                echo "<script> alert('Amount Has Been Added.') </script>";
            }
        }else{
                echo "<script> alert('User Has Not Sufficient Amount.') </script>";
